...
namespace Octree
{
	...
	public unsafe partial struct NativeOctree<T> where T : unmanaged {
		public static void Draw(NativeOctree<T> tree, NativeList<OctElement<T>> results, AABB range,
			...
			for (int i = 0; i < tree._nodes->Capacity; i++)
			{
				var node = UnsafeUtility.ReadArrayElement<OctNode>(tree._nodes->Ptr, i);

				if(node.Count > 0)
				{
					for (int k = 0; k < node.Count; k++)
					{
						var element = UnsafeUtility.ReadArrayElement<OctElement<T>>(tree._elements->Ptr, node.FirstChildIndex + k);
			...
			if (results.IsCreated) {
				foreach (var element in results) {
					...
					texture[(int) ((element.Pos.x + widthAdd) * widthMult)][(int) ((element.Pos.y + heightAdd) * heightMult)]
						= Color.green;
				}
			}
		...
		private static void DrawPoint(OctElement<T> element, Color color) {
			UnityEngine.Debug.DrawLine(element.Pos + (float3) Vector3.left, element.Pos + (float3) Vector3.right,color, 15f);
			UnityEngine.Debug.DrawLine(element.Pos + (float3) Vector3.up, element.Pos + (float3) Vector3.down, color, 15f);
			UnityEngine.Debug.DrawLine(element.Pos + (float3) Vector3.back, element.Pos + (float3) Vector3.forward, color, 15f);
		}
