   ...
   public unsafe partial struct NativeOctree<T> where T : unmanaged {
      private struct OctreeRangeQuery {
         ...
         public void Query(NativeOctree<T> tree, AABB bounds, NativeList<OctElement<T>> results) {
            ...
            _bounds.Extents += *tree._maxRadius;
         ...
         public void RecursiveRangeQuery(AABB parentBounds, bool parentContained, int prevOffset, int depth) {
            if (_count + 8 * _tree._maxLeafElements > _fastResults -> Capacity) {
               _fastResults -> Resize(max(_fastResults -> Capacity * 2, _count + 8 * _tree._maxLeafElements));
            }
            ...
            for (int l = 0; l < 8; l++) {
               ...
               if (contained) {
                  void* index = (void*) ((IntPtr) _tree._elements -> Ptr + node.FirstChildIndex * UnsafeUtility.SizeOf<OctElement<T>>());
                  ...
                  for (int k = 0; k < node.Count; k++) {
                     OctElement<T> element = UnsafeUtility.ReadArrayElement<OctElement<T>>(_tree._elements -> Ptr, node.FirstChildIndex + k);
